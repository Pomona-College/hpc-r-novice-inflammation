...
module load r
Rscript readings.R --mean inflammation-*.csv
```

Submit with `sbatch analysis.sh`. `%j` puts the job ID in the log filename, so
repeated runs do not overwrite each other's output.

Workshop 9, *SLURM Job Scheduling on Sagehen*, covers the options in detail.
