...
R evaluates this expression from the inside out. Reading from left to right, the call stack passes through five distinct states:

1. **Global frame only** — `inner_vec` and `outer_vec` are bound in the global environment. The call stack contains a single frame:

   ```
   [global]  inner_vec = "carbon"
             outer_vec = "+"
   ```

2. **Entering `highlight`** — R needs the *first* argument of `edges`, so it must call `highlight(inner_vec, outer_vec)` first. A new stack frame is pushed for `highlight`. Inside that frame, the formal arguments (let's call them `content` and `wrapper`) bind to the caller's `inner_vec` and `outer_vec` values:

   ```
   [highlight]  content = "carbon"
                wrapper = "+"
   [global]     inner_vec, outer_vec
   ```

3. **`highlight` returns** — the function returns its result (e.g., `"+carbon+"`). R *pops* the `highlight` frame; its locals (`content`, `wrapper`) are gone:

   ```
   [global]  inner_vec, outer_vec
             [returned value: "+carbon+"]
   ```

4. **Entering `edges`** — R now calls `edges("+carbon+")`. A new frame is pushed for `edges`, with its formal argument bound to the returned string:
...
5. **`edges` returns; assignment to `result`** — `edges` produces its return value (e.g., `c("+", "+")`). The `edges` frame is popped, and the return value is bound to `result` in the global environment:
...
- **Frames are pushed in evaluation order, not lexical order.** Even though `edges(...)` appears outermost in the source code, `highlight(...)` is pushed first because R must evaluate the argument before the call.
- **Local variables die with their frame.** If you try to print `content`, `wrapper`, or `v` at the global prompt, R reports "object not found" — those names existed only inside their respective frames.

A handy way to confirm this in practice is to insert `traceback()` inside one of the helper functions, or to set `options(error = recover)` before triggering an error inside `highlight` — R will list the active frames and let you inspect their locals.
