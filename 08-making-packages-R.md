...
home directory — accept, and everything you install afterwards goes there.

```r
install.packages('devtools')   # installs to your personal library
.libPaths()                    # shows where R is actually looking
```

Two things worth knowing. Personal libraries count against your 100 GB `/rhome`
quota, and a large package set adds up. And packages are built against the R
version that installed them, so if you switch R versions you may need to
reinstall — pin `module load r/4.5.1` in scripts you want to keep working.
