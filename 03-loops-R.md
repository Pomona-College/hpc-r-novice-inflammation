...
## When to stop looping and start a job array

Looping over a dozen data files in R is sensible. Looping over several hundred,
one after another, wastes most of the cluster.

Because each file is independent, SLURM can process them at the same time using
a job array — one task per file, each picking its input from
`$SLURM_ARRAY_TASK_ID`:
...
Workshop 18, *Parallel Computing Fundamentals*, covers the pattern. It is
usually the largest speed-up available to anyone processing many files.
